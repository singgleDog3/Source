...
u8 code_chip_erase_opcode;
u8 code_chip_erase_ack_opcode;
...
void lower_assign_opcode()
{
    if(device_type == AUDIO_DEVICE_EEPROM) {
        ...
        code_write_ack_opcode = CODE_WRITE_ACK;
        code_disconn_ack_opcode = CODE_DISCONN_ACK;
        code_chip_erase_opcode = CODE_FLASH_ERASE;
        code_chip_erase_ack_opcode = CODE_ERASE_ACK;
    }
...
void lower_uart_tx_chip_erase(void)
{
    lower_state = LOWER_CHIP_ERASE;
    lower_uart_finish_callback = lower_uart_tx_bin;
    lower_uart_tx(code_chip_erase_opcode, 0, 0, NULL);
}
...
void lower_uart_rx(u8 data)
{
    switch(lower_state) {
        ...
        case LOWER_WRITE:
            ...
            break;
        case LOWER_CHIP_ERASE:
            lower_uart_rx_ack(data, code_chip_erase_ack_opcode);
            break;
